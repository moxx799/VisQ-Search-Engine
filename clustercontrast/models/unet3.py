...
class UNet(nn.Module):
    def __init__(self, depth=0, pretrained=True, cut_at_pooling=False, img_size = 175, 
        ...
        out_planes1 = 512 * val*val*new_in_channels # val*val*new_in_channels # 51200 # 512 *  9#new_in_channels #2560
        out_planes = int(out_planes1/new_in_channels)  # 2500 #512 *  9 #2500 
        # self.FC = nn.Sequential(nn.Linear(out_planes1, out_planes1), nn.Linear(out_planes1, out_planes), nn.Linear(out_planes, out_planes)) 
        
        self.vectorize=nn.Sequential(Flatten(), nn.Linear(out_planes1, out_planes), nn.Linear(out_planes, out_planes))
    ...
    def forward(self, x):
        ...
        x=self.vectorize(x) #linear layer - no summing 
        # print('Sum size: ', x.size() ) # 50x512x100,  batch x3x3
