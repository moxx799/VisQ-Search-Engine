...
class PatchEmbed_overlap(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, stride_size=16, new_in_channels=3, embed_dim=768):
        super().__init__()
        ...
        self.proj = nn.Conv2d(new_in_channels, embed_dim, kernel_size=patch_size, stride=stride_size)
...
class PatchEmbed_VOLO(nn.Module):
    ...
    def __init__(self, img_size=224, stem_conv=False, stem_stride=1,
                 patch_size=8, new_in_channels=3, hidden_dim=64, embed_dim=384):
        super().__init__()
        ...
        if stem_conv:
            self.conv = nn.Sequential(
                nn.Conv2d(new_in_channels, hidden_dim, kernel_size=7, stride=stem_stride,
...
class TransReID(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, stride_size=16, new_in_channels=3, num_classes=0, embed_dim=768, 
        ...
        if conv_stem:
            self.patch_embed = PatchEmbed_VOLO(img_size=img_size,stem_conv=True, stem_stride=2, 
                patch_size=patch_size,new_in_channels=new_in_channels, hidden_dim=64,embed_dim=384)
            print('Using convolution stem')
        else:
            self.patch_embed = PatchEmbed_overlap(
                img_size=img_size, patch_size=patch_size, stride_size=stride_size, new_in_channels=new_in_channels,
                embed_dim=embed_dim)
