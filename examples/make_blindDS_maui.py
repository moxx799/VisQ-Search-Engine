...
# functions
def select_random(carray, num):
    carray.sample(frac=1)[:num]
    return carray.sample(frac=1)[:num]
...
def main(input_dir, bbxs_file, channel_names, dst, inside_box=[8000, 4000, 34000, 24000], topN=7000, margin=5,
    ...
    print('Biomarkers are: ', biomarkers)
    # read bbxs file
    assert os.path.isfile(bbxs_file), '{} not found!'.format(bbxs_file)
    if '.txt' in bbxs_file: 
        table_orig1 = pd.read_csv(bbxs_file, sep='\t')
    else: 
    ...
    # image_size = io.imread(os.path.join(input_dir, biomarkers['NeuN'])).shape
    print('Image shape is: ', image_size)
    ...
    for i, bioM in enumerate(biomarkers.keys()):
        print('Working on: ', bioM)
        if biomarkers[bioM] != "":
            try: 
                images[:, :, i] = io.imread(os.path.join(input_dir, biomarkers[bioM]))
            except: 
                print('Resizing due to issue')
                temp_img = io.imread(os.path.join(input_dir, biomarkers[bioM]))
                image_resized = resize(temp_img, (image_size),anti_aliasing=True)
    ...
    cents = cents[top_cells, :]
    
    bbxs = bbxs[top_cells, :]
    cell_ids = cell_ids[top_cells, :]
...
if __name__ == '__main__':
    ...
    main(args.INPUT_DIR, args.BBXS_FILE, [args.DAPI, args.HISTONES, args.NEUN, args.S100, args.OLIG2, args.IBA1, args.RECA1, args.other1, args.other2, args.other3 ],
            args.OUTPUT_DIR, inside_box=[8000, 4000, 34000, 24000], parallel=True, margin=5, crop_size=(100, 100), topN=5000) # crop_size 175
    print('*' * 50)
    print('*' * 50)
