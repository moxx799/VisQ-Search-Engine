...
class MultiCropTransform:
    """Create 3 crops of the same image"""
...
def get_train_loader(args, dataset, height, width, mean_array, std_array, batch_size, workers,
    ...
    train_transformer2 = T.Compose([
        ...
        T.RandomHorizontalFlip(p=0.5),
        T.Pad(10),
        T.RandomCrop( (int(height*0.75), int(width*0.75) )), #rwm out
        # T.ToTensor(),
        normalizer,
        T.RandomErasing(probability=0.5, mean=[0.1012, 0.1008, 0.0997, 0.0637, 0.0404, 0.0986, 0.0982])
    ])
    train_transformer3 = T.Compose([
        ...
        T.RandomHorizontalFlip(p=0.5),
        T.Pad(10),
        T.RandomCrop( (int(height*0.5), int(width*0.5) )), #rwm out
        # T.ToTensor(),
        normalizer,
        T.RandomErasing(probability=0.5, mean=[0.1012, 0.1008, 0.0997, 0.0637, 0.0404, 0.0986, 0.0982])
    ])
    ...
    train_loader = IterLoader(
        DataLoader(Preprocessor(train_set, root=dataset.images_dir, transform=MultiCropTransform(train_transformer1, train_transformer2, train_transformer3)), 
...
def get_test_loader(dataset, height, width, mean_array, std_array, batch_size, workers, testset=None):
    ...
    test_transformer1 = T.Compose([
        T.ToTensor(), #RWM
        T.CenterCrop(height), #rwm out
        # T.ToTensor(),
        normalizer
    ])
    test_transformer2 = T.Compose([
        T.ToTensor(), #RWM
        T.CenterCrop(int(height*0.5) ), #rwm out
        # T.ToTensor(),
        normalizer
    ])
    test_transformer3 = T.Compose([
        T.ToTensor(), #RWM
        T.CenterCrop(int(height*0.5)), #rwm out
        # T.ToTensor(),
        normalizer
    ...
    test_loader = DataLoader(
        Preprocessor(testset, root=dataset.images_dir, transform=MultiCropTransform(test_transformer1, 
                                                                                    test_transformer2, test_transformer3)),
...
def create_model(args):
    #create an ensemble model with 3 exact same backbones 
    model = models.EnsembleModel(args) # args.arch, num_features=args.features, new_in_channels=args.num_channels, norm=True, dropout=args.dropout, num_classes=0, pooling_type=args.pooling_type)
    # use CUDA
    model.cuda()
    ...
    for epoch in range(args.epochs):
        with torch.no_grad():
            ...
            if epoch == 0:
                ...
                cluster = DBSCAN(eps=eps, min_samples=10, metric='precomputed', n_jobs=-1) #RWM changed from 4 to 20 

            # select & cluster images as training set of this epochs
            pseudo_labels = cluster.fit_predict(rerank_dist)
            num_cluster = len(set(pseudo_labels)) - (1 if -1 in pseudo_labels else 0)

            print("epoch: {} \n pseudo_labels: {}".format(epoch, pseudo_labels.tolist()[:100])) #RWM uncomment
